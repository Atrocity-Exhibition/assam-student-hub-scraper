...
def test_kkhsou_scraper():
    print("=== Launching KKHSOU Scraper Test ===")
    
    scraper = KKHSOUScraper()
    items = scraper.run()
    
    print(f"Scraped Items Count: {len(items)}")
    assert len(items) > 0, "Error: Scraper returned 0 items from KKHSOU!"
    ...
    assert first_item.institution_slug == "krishna-kanta-handiqui-state-open-university", "Institution slug is invalid"
    ...
    print("\n[SUCCESS] KKHSOU scraper verification checks passed successfully!")
