...
def test_assam_job_news_scraper():
    print("=== Launching assamJOBnews Scraper Test ===")
    
    scraper = AssamJobNewsScraper()
    items = scraper.run()
    
    print(f"Scraped Items Count: {len(items)}")
    assert len(items) > 0, "Error: Scraper returned 0 items from assamJOBnews!"
    ...
    assert first_item.institution_slug == "assamjobnews", "Institution slug is invalid"
    assert first_item.reliability_score == 6, "Reliability score should be 6 for aggregator"
    assert len(first_item.content_hash) == 64, "Content hash should be SHA-256 (64 hex characters)"
    
    print("\n[SUCCESS] assamJOBnews scraper verification checks passed successfully!")

if __name__ == "__main__":
    test_assam_job_news_scraper()
