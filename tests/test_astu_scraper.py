...
def test_astu_scraper():
    print("=== Launching ASTU Scraper Test ===")
    
    scraper = ASTUScraper()
    items = scraper.run()
    
    print(f"Scraped Items Count: {len(items)}")
    assert len(items) > 0, "Error: Scraper returned 0 items from ASTU!"
    ...
    assert first_item.institution_slug == "assam-science-and-technology-university", "Institution slug is invalid"
    ...
    print("\n[SUCCESS] ASTU scraper verification checks passed successfully!")
