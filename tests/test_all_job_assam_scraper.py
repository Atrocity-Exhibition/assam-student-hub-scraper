...
def test_all_job_assam_scraper():
    print("=== Launching AllJobAssam Scraper Test ===")
    
    scraper = AllJobAssamScraper()
    items = scraper.run()
    
    print(f"Scraped Items Count: {len(items)}")
    assert len(items) > 0, "Error: Scraper returned 0 items from AllJobAssam!"
    ...
    assert first_item.institution_slug == "all-job-assam", "Institution slug is invalid"
    ...
    print("\n[SUCCESS] AllJobAssam scraper verification checks passed successfully!")

if __name__ == "__main__":
    test_all_job_assam_scraper()
