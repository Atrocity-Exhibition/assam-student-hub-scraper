...
def test_awu_scraper():
    print("=== Launching AWU Scraper Test ===")
    
    scraper = AWUScraper()
    items = scraper.run()
    
    print(f"Scraped Items Count: {len(items)}")
    assert len(items) > 0, "Error: Scraper returned 0 items from AWU!"
    ...
    assert first_item.institution_slug == "assam-womens-university", "Institution slug is invalid"
    ...
    print("\n[SUCCESS] AWU scraper verification checks passed successfully!")
